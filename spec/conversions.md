...
将匿名函数转换为表达式树时，还可以使用此处应用以捕获本地变量的相同技术： 对编译器生成对象的引用可存储在表达式树中，并可以是本地变量的访问权限表示为字段访问这些对象。 此方法的优点是它允许"提升"的本地变量的委托和表达式树之间共享。
...
分配给`d2`演示如何创建具有派生程度较低 （逆变） 参数类型和更派生 （协变） 的返回类型的方法的委托。
