...
记录是C#类类型和结构类型的一个新的简化声明窗体，它们结合了许多更简单的功能的优点。 我们介绍了新功能（调用方-接收方参数和*带有表达式*的），为记录声明提供语法和语义，并提供一些示例。


## <a name="motivation"></a>动机
[motivation]: #motivation

在中C# ，类型声明的数量很少超过类型化数据的聚合集合。 遗憾的是，声明此类类型需要大量的样板代码。 *记录*提供了一种机制，用于通过描述聚合的成员以及其他代码或与常用样本（如果有）的偏差来声明数据类型。
...
当前方法参数的*默认参数*必须是 
- *常数表达式*;或
- 格式 `new S()` 的表达式，其中 `S` 为值类型;或
- 格式 `default(S)` 其中 `S` 为值类型的表达式

我们对此进行了扩展，以添加以下
- 格式 `this.Identifier` 的表达式

此新格式称为*调用方-接收方默认参数*，仅当满足以下所有条件时才允许使用
- 它在其中出现的方法是实例方法;与
- 表达式 `this.Identifier` 绑定到封闭类型的实例成员，该成员必须是字段或属性;与
- 它绑定到的成员（如果它是属性，则为 `get` 访问器）至少与方法一样可访问;与
- `this.Identifier` 的类型可通过标识或可为 null 的转换隐式转换为参数的类型（这是对*默认参数*的现有约束）。
...
>     p = p.With(x: 1);
> ```
> 若要创建新的 `Point`，就像现有 `Point`，但 `X` 的值已更改。
> 
...
对于名*为 "identifier2" 的*可访问实例成员的每个 `With` 名为的方法，我们将选择 " *identifier2* " 作为该方法中第一个参数的名称，该方法具有与实例字段或绑定到*标识符*的属性相同的成员。 如果无法识别此类参数，则不需要考虑方法。 要调用的方法是从其余候选项的重载决策中选择的。
...
有关 `Deconstruct` 规范及其与模式匹配的关系，请参阅[模式匹配规范](csharp-8.0/patterns.md#positional-pattern)。

> **设计说明**：根据此处指定的编译器生成的 `Deconstruct`，以及模式匹配、记录声明的规范
> ```cs
...
`class` 或 `struct` 声明的语法已扩展为支持值参数;参数将变为类型为的属性：
...
对于记录类型声明的每个记录参数，都有一个对应的 `public` 属性成员，其名称和类型取自值参数声明。 其名称是*record_property_name*的*标识符*（如果存在）或*record_parameter*的*标识符*，否则为。 如果没有显式声明或继承具有 `get` 访问器并且具有此名称和类型的具体（即非抽象）公共属性，则编译器将生成，如下所示：

* 对于*record 结构*或 `sealed`*记录类*：
 * 将 `private` `readonly` 字段生成为 `readonly` 属性的支持字段。 在构造过程中，它的值将与相应的主构造函数参数的值一起初始化。
 * 实现该属性的 `get` 访问器以返回支持字段的值。
 * 将重写每个 "匹配" 继承的虚拟属性 `get` 访问器。
...
对于*record 结构*或 `sealed`*记录类*，除非用户提供，否则编译器将生成 `object.GetHashCode()` 和 `object.Equals(object)` 方法的实现。

- []**打开问题**：我们应该精确指定其实现。
- []**打开问题**：我们还应为记录类型添加接口 `IEquatable<T>`，并指定提供实现。
- []**打开问题**：我们还应指定每个 `IEquatable<T>.Equals`实现。
- []**打开问题**：我们应确切地指定我们在记录继承面对的问题的解决方法：具体而言，我们如何生成相等方法，以使它们具有对称、可传递、反身等。
- []**打开问题**：已建议对记录类型实施 `operator ==` 和 `operator !=`。
- []**打开问题**：是否应自动生成 `object.ToString`的实现？

##### `Deconstruct`

记录类型具有编译器生成的 `public` 方法 `void Deconstruct`，除非用户提供具有任何签名的。 每个参数都是一个与该记录类型的相应参数具有相同名称和类型的 `out` 参数。 此方法的编译器提供的实现应将每个 `out` 参数分配给相应属性的值。

请参阅 `Deconstruct`的语义的[模式匹配规范](csharp-8.0/patterns.md#positional-pattern)。

##### <a name="with-method"></a>`With` 方法

除非已声明名为 `With` 的用户声明的成员，否则记录类型具有一个名为 `With` 的编译器提供的方法，该方法的返回类型为记录类型本身，并包含与每个*记录参数*相对应的一个值参数，这些参数在记录类型声明中出现的顺序相同。 每个参数都应具有相应属性的*调用方-接收方默认参数*。

在 `abstract` 记录类中，编译器提供的 `With` 方法是抽象的。 在记录结构或密封记录类中，编译器提供的 `With` 方法是 `sealed`的。 否则，编译器提供的 `With` 方法为 "虚拟"，并且其实现应返回一个新实例，该实例通过将参数作为参数来调用主构造函数以从参数创建新实例，并返回该新实例。

- []**打开问题**：我们还应指定在什么条件下重写或实现从实现的接口继承的虚拟 `With` 方法或 `With` 方法。
- []**打开问题**：我们应指出当继承非虚拟 `With` 方法时，会发生什么情况。

> **设计说明**：由于记录类型默认情况下是不可变的，因此 `With` 方法提供了一种方法来创建新的实例，该实例与现有实例相同，但在给定新值的情况下具有所选属性。 例如，给定
> ```cs
> public class Point(int X, int Y);
> ```
> 存在编译器提供的成员
...
- []**打开问题**： `Equals` 的这种实现在继承时不对称。
- []**打开问题**：记录是否声明 `operator ==` 和 `operator !=`？

> **设计说明**：由于一种记录类型可以从另一种记录类型继承，因此在这种情况下 `Equals` 的实现不是对称的，这是不正确的。 建议以这种方式实现相等性：
> ```cs
>     public bool Equals(Pair other) // for IEquatable<Pair>
>     {
...
> ```
> 派生记录将 `override EqualityContract`。 更具吸引力的替代方法是限制继承。
...
我们认为在6中C#添加了*主构造函数*。 尽管它们与此建议具有相同的语法图面，但我们发现它们的优点低于记录所提供的优势。
